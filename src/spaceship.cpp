...
Vertex leftVertex() {
  Vertex v;
  v.x = spaceship->cx - float(spaceship->length / 2.0f);
...
float collisionRadius() { return float(spaceship->length / (2 * sqrt(3))); }

int health() { return spaceship->health; }

void collide(int asteroidSize) { spaceship->health -= asteroidSize / 10; }
